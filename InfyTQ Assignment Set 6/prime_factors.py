...
def is_prime(num, i):
    #Accepts the number num and num/2 --> i and returns True if the number is prime, 
    #else returns False
    if(i==1):
        return True
    elif(num%i==0):
...
def find_f(num):
    ...
    facts = find_factors(num)
    largest_fact = find_largest_prime_factor(facts)
    return largest_fact

def find_g(num):
    #Accepts the number and returns the sum of the largest prime factors of the 9 
    # consecutive numbers starting from the given number
    sum = 0
